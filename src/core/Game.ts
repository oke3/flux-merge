...
export class Game {
  ...
  private mergeNodes(indexA: number, indexB: number) {
    ...
    const cellSize = 600 / GAME_CONFIG.GRID_SIZE;
    const gridX = Math.max(0, Math.min(GAME_CONFIG.GRID_SIZE - 1, Math.round(newX / cellSize)));
    const gridY = Math.max(0, Math.min(GAME_CONFIG.GRID_SIZE - 1, Math.round(newY / cellSize)));
